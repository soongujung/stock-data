...
if __name__ == '__main__':
    ...
    # --  api 크롤링
    url = "http://ecos.bok.or.kr/api/StatisticSearch/[api키]/json/kr/1/5000/005Y003/MM/20140101/20191231/BECBLA03/"
    ...
    df_household_loan_insert = DataFrame(arr_data, columns=arr_columns)
    print('df_kospi_insert : ' + df_household_loan_insert)

    # -- database insert/select
    df_household_loan_insert.to_sql(name='economy_household_loan_month',
                                    con=alchemy_conn,
                                    index=False,
                                    if_exists='replace',  #{'fail', 'replace', 'append'}, default : fail
                                    schema='public')

    df_household_loan = pandas_sql.read_sql_query("select * from economy_household_loan_month", alchemy_conn)
    print("database result ::: stock_kospi_month")
    print(df_household_loan)
